...
rStream.on("data", (b) => {
  ...
  // file stream
  const bStr = b.toString(); // We split the string by spaces and new lines and add it to the // total-- we subtract one because of the extra space/newline/broken word // at the end of the chunk // we shouldn't do this for the last chunk of data, which we handle later
  total += bStr.split(/[\s\n]+/).length - 1;
});
rStream.on("end", () => {
  ...
  console.log("total words:", total + 1); // Print the total time taken, as well as the total used program memory
